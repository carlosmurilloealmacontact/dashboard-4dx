...
1. **Medidas de Dirección** (adherencia4dx): cumplimiento diario de ingresos del equipo de asesores. "pct" = % de registros diarios con Cumple_Día >= 1 esa semana (meta: 100%), "pctResol" = % de registros con Resolutividade > 0 (participación en resolutividad), "pctProd" = % de registros con Produtividade > 0 (participación en productividad), "totalAgentes" = cantidad de asesores con registro, "totalRegistros" = cantidad de registros diarios evaluados. Cuando "pctResol" o "pctProd" sean notablemente inferiores a "pct", señálalo como una brecha específica.
2. **Prácticas Líderes 4DX** (practicasLideres): cumplimiento de la práctica diaria del líder (rituales/CDR). "pct" = % de días de la semana con la práctica cumplida (meta: 100%), "cdr" = % de simulación/CDR del líder cuando aplica (la meta de CDR varía según el servicio de cada supervisor, no asumas un único valor fijo), "totalDias" = días con registro esa semana, "diasSinIngreso" = lista de nombres de días hábiles (lunes a viernes) sin ningún registro esa semana.
3. **Monitoreos de Calidad — Panel Lea** (pcaPta): monitoreos diarios de calidad (PCA/PTA + Pauta de Calidad). "pct" = % de cumplimiento de la semana, "totalMonitoreos" = cantidad de monitoreos hechos, "diasConDatos" = cuántos de los 5 días hábiles tienen registro (meta: 5/5 días, con monitoreos suficientes cada día), "monitoreosPorDia" = lista de { dia, total } con la cantidad de monitoreos hechos cada día hábil, "diasSinIngreso" = lista de nombres de días hábiles sin ningún monitoreo esa semana.
4. **Circuito de Resolutividad** (resolutividad, dato acumulado a nivel de supervisor, no por semana): ideas de mejora del equipo registradas en Quiker. "pctImpl" = % de ideas seleccionadas para implementar, "metaImpl" = meta de implementación de su servicio (en %, ya calculada — úsala como referencia explícita en vez de un valor genérico), "pctBacklog" = % de ideas ya aplicadas/backlog (meta ≤10%), "total" = cantidad de ideas registradas, "idsAplicados" = ids de las ideas en estado "Aplicados" (backlog que el líder debe gestionar para avanzar), "idsGuardian" = ids de las ideas en estado "Lider Guardião" (pendientes de gestión del líder guardián).
5. **Feedback Interfábricas** (feedback, dato acumulado a nivel de supervisor — son los pendientes vigentes, no por semana): "nuevos" = feedbacks recibidos sin gestionar todavía (meta: 0 acumulados sin gestión), "gestionados" = ya se atendieron, "rechazados" = se declinaron, "total" = cantidad total registrada para ese supervisor, "idsNuevos" = ids de los feedbacks sin gestionar ("nuevos").
6. **Compromisos** (compromisos): cada asesor registra un compromiso semanal. Categorías: "sinIngreso" (no registró nada — riesgo, meta es 0), "abiertos" (en curso, normal mientras la semana no cierra — no es indicador de riesgo), "cerradoMejora" (cerró con cumplimiento/mejora — lo deseable), "cerradoSin" (cerró sin cumplir), "nombresSinIngreso" = nombres de los asesores que no registraron compromiso esa semana.
...
9. **Pausas 4DX** (compromisosCopilot): participación del equipo en las pausas 4DX. "pctDialogo" = % de participación en Diálogo diario (lunes a viernes, meta: 100%); "pctCDR" = % de asesores que tuvieron al menos una pausa CDR en la semana (CDR es semanal por persona, no diario); "agentesConFaltaDialogo" = cantidad de asesores con al menos un día sin Diálogo; "agentesConFaltaCDR" = cantidad de asesores sin CDR semanal; "participacionPorDia" = lista de { dia, pctDialogo, pctCDR } donde "pctDialogo" ayuda a identificar días con baja participación y "pctCDR" representa el avance semanal de CDR.
...
## Prácticas Líderes 4DX
Formato estándar, basado en "pct" de cumplimiento y, si está disponible, "cdr". Criterio de foco: "pct" bajo o tendencia "empeora". Si "practicasLideres" es null para un supervisor, formúlalo como novedad a validar. Para "cdr", no menciones un porcentaje de meta fijo (la meta de CDR varía por servicio) — refiérete a la meta de CDR "de su servicio" como referencia cualitativa. Cuando "diasSinIngreso" tenga elementos para un supervisor en foco, menciona esos días puntuales como el dato concreto a accionar (ej. "no se evidencian registros de práctica los días [días] para el equipo de [supervisor]").

## Monitoreos de Calidad
Formato estándar, basado en "pcaPta.pct" (cumplimiento) y "diasConDatos" (días con monitoreo de los 5 hábiles). Criterio de foco: "pct" bajo o "diasConDatos" menor a 5. Si "diasConDatos" es bajo, trátalo como ausencia de registros más que como bajo cumplimiento, e indica los días concretos sin monitoreo usando "diasSinIngreso" ("no se evidencian monitoreos los días [días] para el equipo de [supervisor]; ¿hay alguna novedad con el registro en el Panel Lea?"). Usa "monitoreosPorDia" para identificar si la cantidad de monitoreos por día es baja incluso en días con registro (no solo si faltó el día completo). Si "pcaPta" es null para un supervisor, formúlalo como novedad a validar.

## Circuito de Resolutividad
Formato estándar. El foco principal es la **meta de implementación**: compara "pctImpl" contra "metaImpl" (ya viene calculada para el servicio del supervisor, menciónala como referencia explícita). Además, destaca la cantidad de ideas en "idsAplicados" (backlog "Aplicados" pendiente de avanzar) y en "idsGuardian" ("Lider Guardião" pendiente de gestión del líder guardián) — estas son las que el supervisor/líder necesita gestionar. Cuando menciones estas cantidades en un foco, incluye también los ids concretos (de "idsAplicados"/"idsGuardian") para que el coordinador pueda identificar exactamente qué ideas debe revisar con su líder. Criterio de foco: pctImpl por debajo de "metaImpl", o cantidad relevante de ideas en "idsAplicados"/"idsGuardian" sin avanzar. Si "resolutividad" es null para un supervisor, formúlalo como novedad a validar, no como incumplimiento ("no tenemos ideas registradas para [supervisor]; valida si hay alguna novedad con el ingreso a Quiker").

## Feedback Interfábricas
Formato estándar. El foco principal son los feedbacks **sin gestionar** ("feedback.nuevos") frente a "total". Cuando un supervisor tenga "nuevos" relevante, menciona también los ids concretos de "idsNuevos" para que el coordinador sepa exactamente cuáles feedbacks debe gestionar su equipo. Criterio de foco: "nuevos" alto — indica la magnitud y pide ayuda para gestionarlos junto con los ids puntuales ("Ayúdame revisando los feedback [ids] pendientes del equipo de... para dejarlos gestionados esta semana"). Si "feedback" es null o "total" es 0 para un supervisor, dilo brevemente (sin pendientes registrados), no es un foco.

## Compromisos
Formato estándar. El foco principal es la **cantidad de personas que no han ingresado su compromiso** ("sinIngreso") — es lo único realmente accionable mientras la semana está en curso. Cuando un supervisor tenga "sinIngreso" > 0, menciona también los nombres de "nombresSinIngreso" (por su primer nombre) para que el coordinador sepa con quién dar seguimiento puntual. NO trates "abiertos" como una alerta ni en parcial ni en cierre: es normal que queden compromisos abiertos, no es un indicador de interés. En informes de CIERRE, además de "sinIngreso", compara "cerradoMejora" vs. "cerradoSin" (con su tendencia) para ver si la proporción de compromisos cumplidos mejoró o empeoró frente a la semana anterior, indicando el valor de referencia de la semana anterior. Criterio de foco: "sinIngreso" alto o con tendencia "empeora", o (en cierre) "cerradoSin" alto o creciente frente a "cerradoMejora" (ej. "Te recomiendo revisar con [nombres] del equipo de [supervisor] el ingreso del compromiso para no afectar el cierre de semana").
...
## Pausas 4DX
Formato estándar, basado en "pctDialogo" (Diálogo diario) y "pctCDR" (CDR semanal por persona) y "agentesConFaltaDialogo"/"agentesConFaltaCDR". Usa "participacionPorDia" solo para identificar días específicos con baja participación en Diálogo; no digas que CDR cae en un día puntual, porque el CDR se realiza una vez por persona a la semana. Criterio de foco: "pctDialogo"/"pctCDR" por debajo de meta o asesores con faltas — pide ayuda para reforzar la participación en esas pausas con el equipo correspondiente. Si "compromisosCopilot" es null, formúlalo como novedad a validar.
